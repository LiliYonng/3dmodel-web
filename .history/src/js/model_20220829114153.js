...
function init() {
  ...
  let textureOption = document.getElementById("icon");
  textureOption.addEventListener("click", function (e) {
  ...
  //select切换模型
  const modelOption = document.getElementById("modelName");
  ...
  modelOption.addEventListener("change", function () {
    ...
    modelAry.forEach((mymodel, myindex) => {
      if (mymodel.modelName == modelName) {
        index = myindex;
        modelType = mymodel.type;
      }
    });

    changeModel(index); //传入选择的模型名称
    changeIcon(index); //修改贴图icon为对应模型
    changeBg("defalut");
    //切换模型后清空文字描述
    document.getElementById("content").innerText = "";
    document.getElementById("texInfo").innerText = "";
    flag = true;
    //render();
  });
}
