<template lang="">
  <div class="box"> <!-- <div class="intrBtn"> <button class="textIntr">文字介绍</button>
  <button class="audioIntr">语音介绍</button> </div> <div class="left_aside"> < </div>
  <div class="right_aside"> > </div> --> <div id="container"></div> <div
  class="ctr_pannel"> <div class="texuBtn" @click="changeMap('color2')"> <img
  src="/model/icon/color_icon.jpg" id="color" /> <img src="/model/icon/specular_icon.jpg"
  id="specular" /> <img src="/model/icon/gilt_icon.jpg" id="gilt" /> </div> </div> </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadObj: function () {
      ...
      let thePath = this.curModel.path + this.curModel.name;
  ...
  watch:{
    modelInfo(){
        this.curModel =this.modelInfo;
        this.changing();
    }
  },
  mounted() {
    this.curModel = this.modelAry[0];
    if(this.curModel)
    {
      this.init();
      this.render();
